...
The approved Launch composition has a mapping scoped to `.launch-homepage`: system sans-serif typography, bright mint-neutral canvas and surfaces, a clear teal accent, restrained warm supporting media, intermediate section spacing, and softer card/media radii. Text/action contrast and focus indicators passed the relevant WCAG 2.2 AA thresholds, and the mapping remained isolated from Premium, Manufacturing, and global defaults.

Phase 7 adds a theme-neutral site shell in Review. `SiteHeader-001`, `SiteFooter-001`, `PageIntro-001`, and `ContactOptions-001` consume the same semantic tokens as the selected page composition. The shell uses a native disclosure for mobile navigation, an always-visible desktop navigation, current-page state, a skip link, and 44px interaction targets without hydration.
